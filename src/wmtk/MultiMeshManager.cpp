...
void MultiMeshManager::write_tuple_map_attribute(
...
std::tuple<Tuple, Tuple> MultiMeshManager::read_tuple_map_attribute(
    MeshAttributeHandle<long> map_handle,
    ...
    const Tuple& source_tuple)
{
    ...
    return std::make_tuple(
        Tuple(map(0), map(1), map(2), map(3), map(4)),
        Tuple(map(5), map(6), map(7), map(8), map(9)));
}
    ...
    const std::vector<std::array<Tuple, 2>>& child_mesh_simplex_map)
{
    ...
    auto map_to_parent_handle =
        child_mesh->register_attribute<long>("map_to_parent", map_ptype, 10);

    auto map_to_child_handle = parent_mesh.register_attribute<long>(
        fmt::format("map_to_child_{}", cur_child_id),
        map_ptype,
        10);
    for (long id = 0; id < parent_mesh.capacity(map_ptype); ++id) {
        write_tuple_map_attribute(
            map_to_child_handle,
            parent_mesh,
            parent_mesh.tuple_from_id(map_ptype, id),
            Tuple());
    }
    // register maps
    for (long id = 0; id < child_mesh->capacity(map_ptype); ++id) {
        write_tuple_map_attribute(
            ...
            *child_mesh,
            child_mesh_simplex_map[id][0],
            child_mesh_simplex_map[id][1]);
    ...
    child_mesh->multi_mesh_manager.m_child_id =
        long(parent_mesh.multi_mesh_manager.child_meshes.size());
    ...
    for (long child_cell_id = 0; child_cell_id < long(child_mesh_simplex_id_map.size());
        ...
        child_mesh_simplex_map.push_back(
            {child_mesh->tuple_from_id(map_type, child_cell_id),
             parent_mesh.tuple_from_id(map_type, parent_cell_id)});
    }
...
Tuple MultiMeshManager::map_tuple_between_meshes(
    ...
    MeshAttributeHandle<long> source_map_handle,
    const Tuple& source_tuple)
{
    ...
    const Simplex& simplex_parent)
{
    ...
    auto top_simplex_in_open_star =
        SimplicialComplex::open_star(parent_mesh, simplex_parent).get_simplices(childmesh_ptype);
    ...
    for (auto s : top_simplex_in_open_star) {
        auto child_tuple =
            map_tuple_between_meshes(parent_mesh, *child_mesh_ptr, map_to_child_handle, s.tuple());
    ...
    const Tuple& edge_tuple)
{
    ...
    for (auto child_mesh_ptr : parent_mesh.multi_mesh_manager.child_meshes) {
        ...
        auto map_to_child_handle = parent_mesh.multi_mesh_manager.map_to_child_handles[child_id];
        Tuple child_tuple =
            map_tuple_between_meshes(parent_mesh, *child_mesh_ptr, map_to_child_handle, edge_tuple);
...
bool MultiMeshManager::is_map_valid(const Mesh& parent_mesh) const
{
    for (auto child_mesh_ptr : child_meshes) {
        ...
        for (long id = 0; id < child_mesh_ptr->capacity(map_type); ++id) {
            if (child_cell_flag_accessor.scalar_attribute(
                    child_mesh_ptr->tuple_from_id(map_type, id)) == 0) {
                continue;
            }
            ...
            auto [child_tuple, parent_tuple] = read_tuple_map_attribute(
                child_to_parent_handle,
                *child_mesh_ptr,
                child_mesh_ptr->tuple_from_id(map_type, id));
            ...
            auto [parent_tuple_test, child_tuple_test] =
                read_tuple_map_attribute(parent_to_child_handle, parent_mesh, parent_tuple);
            ...
            if (map_type == PrimitiveType::Face &&
                parent_mesh.top_simplex_type() == PrimitiveType::Face) {
                ...
                for (int i = 0; i < 3; i++) {
                    ...
                    cur_parent_tuple =
                        parent_mesh.switch_edge(parent_mesh.switch_vertex(cur_parent_tuple));
                }
            } else {
