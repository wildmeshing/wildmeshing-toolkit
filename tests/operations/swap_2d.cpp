...
TEST_CASE("split_face_with_attribute", "[operations][split][2D][.]")
{
    throw std::runtime_error("new transfer not implemented");
    //    0---1---2
    //   / \ / \ / \ .
    //  3---4---5---6
    //   \ / \ / \ /
...
TEST_CASE("split_single_triangle_at_mid_point", "[operations][split][2D][.]")
{
    throw std::runtime_error("new transfer not implemented");
    //         0
    //        / \ .
    ...
    CHECK(m.id(m.switch_vertex(m.switch_edge(ret)), PV) == 2);
    CHECK(
        simplex::link(m, Simplex::vertex(m, ret)).simplex_vector(PrimitiveType::Vertex).size() ==
        3);
...
TEST_CASE("split_single_triangle_at_mid_point_with_tag_embedding_on", "[operations][split][2D][.]")
{
    throw std::runtime_error("new transfer not implemented");
    //    0---1---2
    //   / \ / \ / \ .
...
TEST_CASE("split_single_triangle_at_mid_point_with_tag_embedding_off", "[operations][split][2D][.]")
{
    throw std::runtime_error("new transfer not implemented");
    DEBUG_TriMesh m = single_triangle();
    ...
    const Tuple e20 = m.switch_edge(m.switch_face(m.switch_vertex(m.switch_edge(return_tuple))));
...
TEST_CASE("should fail with todo tag 0", "[operations][split][2D][.]")
{
    throw std::runtime_error("new transfer not implemented");
    DEBUG_TriMesh m = single_equilateral_triangle(3);
