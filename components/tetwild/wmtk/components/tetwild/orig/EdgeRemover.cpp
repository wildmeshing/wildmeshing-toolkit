...
void EdgeRemover::init()
{
    ...
    for (unsigned int i = 0; i < edges.size(); i++) {
        ...
        setIntersection(
            tet_vertices[edges[i][0]].conn_tets,
            tet_vertices[edges[i][1]].conn_tets,
            t_ids);
        addNewEdge(edges[i]);
    }
...
bool EdgeRemover::removeAnEdge_32(int v1_id, int v2_id, const std::vector<int>& old_t_ids)
{
    ...
    *it = v_ids[0];
    ...
    if (isFlip(new_tets)) return false;
    ...
    for (int i = 0; i < old_t_ids.size(); i++) {
        for (int j = 0; j < 4; j++) {
            if (tets[old_t_ids[i]][j] == v1_id || tets[old_t_ids[i]][j] == v2_id) {
                std::array<int, 3> tmp = {
                    {tets[old_t_ids[i]][(j + 1) % 4],
                     tets[old_t_ids[i]][(j + 2) % 4],
    ...
    for (int i = 0; i < new_tets.size(); i++) {
        for (int j = 0; j < 3; j++) {
            ...
            if (e[0] > e[1]) e = {{e[1], e[0]}};
            if (!isLocked_ui(e)) es.push_back(e);
...
{
    const int N = 4;
    if (old_t_ids.size() != N) return false;
    ...
    for (int i = 0; i < 2; i++) {
        ...
        if (isFlip(tmp_new_tets)) continue;
        tmp_timer.start();
...
{
    if (old_t_ids.size() != 5) return false;
    ...
    for (int i = 0; i < n12_v_ids.size(); i++) {
        if (!is_v_valid[(i + 1) % 5] && !is_v_valid[(i - 1 + 5) % 5]) continue;
    ...
    if (std::count(is_v_valid.begin(), is_v_valid.end(), true) == 0) return false;

    int selected_id = -1;
    for (int i = 0; i < is_v_valid.size(); i++) {
        if (!is_v_valid[i]) continue;
    ...
    for (int i = 0; i < new_t_ids.size(); i++) {
        for (int j = 0; j < 4; j++) {
            ...
            if (tets[new_t_ids[i]][j] != v1_id && tets[new_t_ids[i]][j] != v2_id &&
                ...
                std::array<int, 3> tmp = {
                    {tets[new_t_ids[i]][(j + 1) % 4],
                     tets[new_t_ids[i]][(j + 2) % 4],
                     tets[new_t_ids[i]][(j + 3) % 4]}};
                std::sort(tmp.begin(), tmp.end());
                auto it = std::find(fs.begin(), fs.end(), tmp);
                if (it != fs.end()) is_surface_fs[new_t_ids[i]][j] = is_sf_fs[it - fs.begin()];
            }
        }
    }
...
bool EdgeRemover::isSwappable_cd1(const std::array<int, 2>& v_ids)
{
    ...
    if (isEdgeOnSurface(v_ids[0], v_ids[1], t_ids)) return false;
    if (isEdgeOnBbox(v_ids[0], v_ids[1], t_ids)) return false;

    return true;
}

bool EdgeRemover::isSwappable_cd1(
    const std::array<int, 2>& v_ids,
    std::vector<int>& t_ids,
    bool is_check_conn_tet_num)
{
    //    std::vector<int> t_ids;
    setIntersection(tet_vertices[v_ids[0]].conn_tets, tet_vertices[v_ids[1]].conn_tets, t_ids);

    if (is_check_conn_tet_num)
        if (t_ids.size() < 3 || t_ids.size() > 5) return false;
    if (isEdgeOnSurface(v_ids[0], v_ids[1], t_ids)) return false;
    if (isEdgeOnBbox(v_ids[0], v_ids[1], t_ids)) return false;
...
void EdgeRemover::getNewTetSlots(int n, std::vector<int>& new_conn_tets)
{
    ...
    if (cnt < n) {
        for (unsigned int i = 0; i < n - cnt; i++) new_conn_tets.push_back(tets.size() + i);
