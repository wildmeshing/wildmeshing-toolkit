        ...
        if (after(unmods, mods)) {
            ...
            if (m_record && operation_name != "MeshConsolidate") {
                ...
                if (mesh().top_simplex_type() == PrimitiveType::Triangle) {
                    ...
                    auto [F_before, V_before, id_map_before, v_id_map_before] = mesh().parent_scope(
                    ...
                    if (operation_name == "EdgeCollapse") {
                        ...
                        if (dbarea_before.minCoeff() < 0) {
                            ...
                            throw std::runtime_error("collapse negative area in F_before detected");
                            scope.mark_failed();
                            return {};
                        }

                        if (dbarea_after.minCoeff() < 0) {
                            throw std::runtime_error("collapse negative area in F_after detected");
                            scope.mark_failed();
                            return {};
                        }
                        ...
                        if (operation_name == "EdgeSplit") {
                            ...
                            if (V_before.cols() == 2) {
                                ...
                                if (is_boundary_edge) {
                                    ...
                                    Eigen::Vector3d u = eigenvectors.col(2).normalized(); // largest
                                    Eigen::Vector3d v =
                                    ...
                                    for (int i = 0; i < V_before.rows(); ++i) {
                                        Eigen::Vector3d p = V_before.row(i) - centroid.transpose();
                                        V_before_2d.row(i) = Eigen::Vector2d(p.dot(u), p.dot(v));
                                    }
                                    ...
                                    for (int i = 0; i < V_after.rows(); ++i) {
                                        ...
                                        if (it != v_id_map_before.end()) {
                                            ...
                                            V_after_2d.row(i) = Eigen::Vector2d(p.dot(u), p.dot(v));
                                        }
                                    }
                                    ...
                                    if (F_before.rows() == 2) {
                                        ...
                                        auto place_vertex_2d_with_orientation =
                                            ...
                                            double d1 =
                                                (V_before.row(vertex_idx) - V_before.row(ref_v1))
                                                    .norm();
                                            double d2 =
                                                (V_before.row(vertex_idx) - V_before.row(ref_v2))
                                                    .norm();
                                            ...
                                            double cos_angle =
                                                (d1 * d1 + baseline_len * baseline_len - d2 * d2) /
                                                (2 * d1 * baseline_len);
                                            ...
                                            // to ref_v2)
                                            Eigen::Vector2d baseline_dir =
                                                Eigen::Vector2d(ref_x2 - ref_x1, ref_y2 - ref_y1)
                                                    .normalized();
                                            ...
                                            if (v0 == vertex_idx) {
                                                ...
                                                double area_pos =
                                                    compute_signed_area(pos_positive, v1_2d, v2_2d);
                                                double area_neg =
                                                    compute_signed_area(pos_negative, v1_2d, v2_2d);
                                                return (area_pos > 0) ? pos_positive : pos_negative;
                                                ...
                                                double area_pos =
                                                    compute_signed_area(v0_2d, pos_positive, v2_2d);
                                                double area_neg =
                                                    compute_signed_area(v0_2d, pos_negative, v2_2d);
                                                return (area_pos > 0) ? pos_positive : pos_negative;
                                            } else { // v2 == vertex_idx
                                                v0_2d = (v0 == ref_v1)
                                                            ? Eigen::Vector2d(ref_x1, ref_y1)
                                                ...
                                                double area_pos =
                                                    compute_signed_area(v0_2d, v1_2d, pos_positive);
                                                double area_neg =
                                                    compute_signed_area(v0_2d, v1_2d, pos_negative);
                                                return (area_pos > 0) ? pos_positive : pos_negative;
                                            }
                                        };
                                        ...
                                        // for its triangle
                                        V_before_2d.row(page_v1) = place_vertex_2d_with_orientation(
                                            page_v1,
                                            spine_v1,
                                            spine_v2,
                                            0,
                                        ...
                                        // for its triangle
                                        V_before_2d.row(page_v2) = place_vertex_2d_with_orientation(
                                            page_v2,
                                            spine_v1,
                                            spine_v2,
                                            0,
                    ...
                    // DEBUG:
                    if (false) {
                        auto [F_all, V_all, F_flag] = static_cast<TriMesh&>(mesh()).get_FV_Fflag();
                        igl::writeOBJ(
