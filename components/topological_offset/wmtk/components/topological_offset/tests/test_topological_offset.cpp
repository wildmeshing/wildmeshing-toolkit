...
/**
 * @brief End-to-end offset of a disk, where the answer is analytic.
 *
 * A disk of radius R offset by d is exactly an annulus, so the closed region -- input disk plus
 * offset band -- has area pi*(R+d)^2. That turns the whole pipeline into a number instead of a
 * picture, and it is the smallest input that catches two failures seen in practice: the offset
 * capping after a single cell layer and returning the same result for any requested distance,
 * and the region losing manifoldness.
 *
 * d is 1% of the radius, which is the demanding end -- the band is only a few cells wide, so a
 * cap would show up as a large area error rather than a subtle one.
 *
 * The mesh is built here rather than loaded so the test needs no data files. It is a polar
 * triangulation, deliberately coarse in angle (the band is thin radially, which is what the
 * measurement is sensitive to) to keep the run inside a unit test's budget.
 */
TEST_CASE("disk_offset_1pct", "[offset][2d]")
{
    constexpr double R_IN = 0.20; // input disk
    constexpr double R_DOM = 0.22; // domain, just larger than R_IN + d
    constexpr double D = 0.002; // 1% of R_IN
    constexpr int M = 220; // radial rings -> spacing ~1e-3, so the band is ~2 cells
    constexpr int NT = 640; // angular divisions: arc spacing ~= the band width

    // polar node grid: centre, then M rings
    std::vector<std::array<double, 2>> pts{{{0.0, 0.0}}};
    std::vector<std::vector<int>> ring(M);
    for (int k = 1; k <= M; ++k) {
        const double r = R_DOM * k / M;
        for (int a = 0; a < NT; ++a) {
            const double th = 2.0 * M_PI * a / NT;
            ring[k - 1].push_back(static_cast<int>(pts.size()));
    ...
    for (int a = 0; a < NT; ++a) {
        tris.push_back({{0, ring[0][a], ring[0][(a + 1) % NT]}});
    }
    for (int k = 0; k + 1 < M; ++k) {
        for (int a = 0; a < NT; ++a) {
            const int b = (a + 1) % NT;
            tris.push_back({{ring[k][a], ring[k + 1][a], ring[k + 1][b]}});
            tris.push_back({{ring[k][a], ring[k + 1][b], ring[k][b]}});
        }
    }

    MatrixXd V(pts.size(), 2);
    for (size_t i = 0; i < pts.size(); ++i) V.row(i) << pts[i][0], pts[i][1];
    MatrixXi F(tris.size(), 3);
    MatrixSi Tags(tris.size(), 1);
    for (size_t i = 0; i < tris.size(); ++i) {
        F.row(i) << tris[i][0], tris[i][1], tris[i][2];
        double cx = 0, cy = 0;
        for (int j = 0; j < 3; ++j) {
            cx += pts[tris[i][j]][0] / 3.0;
            cy += pts[tris[i][j]][1] / 3.0;
        }
        Tags.coeffRef(i, 0) = (cx * cx + cy * cy < R_IN * R_IN) ? 1 : 0;
    }

    Parameters param;
    param.target_distance = D;
    param.target_distance_rel = -1.0;
    // 0.1, the spec default, rejects nearly every growth candidate at this distance and the
    // offset stops after one cell layer. See tools/README.md.
    param.relative_ball_threshold = 0.01;
    param.offset_in = false;
    param.offset_out = true;
    param.respect_all_topologies = false;
    param.edge_search_term_len = 1e-3;
    param.sorted_marching = false;
    param.optimize = false;
    param.offset_output_tag = {"tag_1"};
    param.init(V.colwise().minCoeff(), V.colwise().maxCoeff());

    TopoOffsetTriMesh mesh(param, 0);
    MatrixXd V_env;
    MatrixXi F_env;
    mesh.init_from_image(V, F, Tags, V_env, F_env, {"tag_0"});
    mesh.m_offset_params.offset_selection =
        components::simwild::expression_parser::parse("tag_0", mesh.m_tag_name_to_id);
    mesh.label_input_complex();
    REQUIRE_FALSE(mesh.empty_input_complex());
    mesh.init_input_complex_bvh();
    mesh.consolidate_mesh();

    mesh.execute_offset("");

    // The closed region is a disk of radius R_IN + D. Measure its area directly.
    double area = 0.0;
    for (const TriMesh::Tuple& f : mesh.get_faces()) {
        const size_t fid = f.fid(mesh);
        if (!mesh.face_in_region(fid)) continue;
    ...
    const double r_out = std::sqrt(area / M_PI);

    // A cap after one cell layer -- the regression this exists to catch -- lands r_out near
    // R_IN + 1e-3, i.e. half the band missing, so it fails this by a wide margin. The bound
    // itself only has to cover polar discretization of a band a couple of cells wide.
    REQUIRE(r_out > R_IN);
    REQUIRE(std::abs(r_out - (R_IN + D)) < 0.3 * D);

    // The offset must be a manifold region, before any optimization touches it.
    REQUIRE(mesh.offset_is_manifold());
}
