...
// ---------------------------------------------------------------------------------------------
// TangentAMIPSEnergy: the real triangles' AMIPS, over a plane. See the header for the reduction
// of wmtk::AMIPS2D_energy to (sum of squared edge lengths) / (2 sqrt(3) area) that makes it one.
// ---------------------------------------------------------------------------------------------

TangentAMIPSEnergy::TangentAMIPSEnergy(
    ...
    , m_t1(std::move(t1))
    , m_t2(std::move(t2))
    , m_n_out(std::move(n_out))
    , m_weight(weight)
{}

bool TangentAMIPSEnergy::cell_at(
    ...
    double& n) const
{
    const Eigen::Vector3d da = p - c.a;
    const Eigen::Vector3d db = p - c.b;
    S = da.squaredNorm() + db.squaredNorm() + (c.a - c.b).squaredNorm();
    N = (c.a - p).cross(c.b - p);
    n = N.norm();
    // The area is unsigned, so the side the triangle faces is a separate question and this is
    // where it is asked: past perpendicular to n_out the energy would go on reporting a perfectly
    // good shape for a folded triangle.
    return n > 0. && std::isfinite(n) && N.dot(m_n_out) > 0.;
}

double TangentAMIPSEnergy::value(const TVector& x)
{
    const Eigen::Vector3d p = point(x);
    double E = 0.;
    double S, n;
    Eigen::Vector3d N;
    for (const Cell& c : m_cells) {
        if (!cell_at(p, c, S, N, n)) return std::nan("");
        E += S / (std::sqrt(3.) * n);
    }
    return m_weight * E;
}

void TangentAMIPSEnergy::gradient(const TVector& x, TVector& gradv)
{
    // Per cell, in p: grad S = 2 (2p - a - b), and N = a x b + p x (a - b) is affine in p with
    // constant Jacobian K = -[a - b]_x, so grad n = K^T N / n. Then
    //     grad (S / n) = grad S / n - S grad n / n^2,
    // and the plane's two coordinates are [t1 t2]^T times that.
    const Eigen::Vector3d p = point(x);
    Eigen::Vector3d G = Eigen::Vector3d::Zero();
    double S, n;
    Eigen::Vector3d N;
    for (const Cell& c : m_cells) {
        if (!cell_at(p, c, S, N, n)) {
            gradv = Eigen::Vector2d::Zero(); // invalid: the line search never accepts this point
            return;
        }
        const Eigen::Matrix3d K = -cross_matrix(c.a - c.b);
        const Eigen::Vector3d dS = 2. * (2. * p - c.a - c.b);
        const Eigen::Vector3d dn = K.transpose() * N / n;
        G += (dS / n - S * dn / (n * n)) / std::sqrt(3.);
    }
    Eigen::Vector2d g;
    g << m_t1.dot(G), m_t2.dot(G);
    gradv = m_weight * g;
}

void TangentAMIPSEnergy::hessian(const TVector& x, MatrixXd& hessian)
{
    // Exact, not Gauss-Newton. With H_S = 4 I and H_n = (K^T K) / n - (grad n)(grad n)^T / n,
    //     H(S / n) = H_S / n - (grad S grad n^T + grad n grad S^T) / n^2
    //                - S H_n / n^2 + 2 S (grad n)(grad n)^T / n^3,
    // then projected as [t1 t2]^T H [t1 t2]. AMIPS is not convex, so this can be indefinite; the
    // solver's own regularisation deals with that, as it does for every other AMIPS here.
    const Eigen::Vector3d p = point(x);
    Eigen::Matrix3d H = Eigen::Matrix3d::Zero();
    double S, n;
    Eigen::Vector3d N;
    for (const Cell& c : m_cells) {
        if (!cell_at(p, c, S, N, n)) continue; // invalid point: contribute nothing
        const Eigen::Matrix3d K = -cross_matrix(c.a - c.b);
        const Eigen::Vector3d dS = 2. * (2. * p - c.a - c.b);
        const Eigen::Vector3d dn = K.transpose() * N / n;
        const Eigen::Matrix3d Hn = (K.transpose() * K) / n - (dn * dn.transpose()) / n;
        Eigen::Matrix3d Hc = 4. * Eigen::Matrix3d::Identity() / n;
        Hc -= (dS * dn.transpose() + dn * dS.transpose()) / (n * n);
        Hc -= S * Hn / (n * n);
        Hc += (2. * S / (n * n * n)) * (dn * dn.transpose());
        H += Hc / std::sqrt(3.);
    }
    Eigen::Matrix<double, 3, 2> T;
    T.col(0) = m_t1;
    T.col(1) = m_t2;
    hessian = m_weight * (T.transpose() * H * T);
}

bool TangentAMIPSEnergy::is_step_valid(const TVector& /*x0*/, const TVector& x1)
{
    const Eigen::Vector3d p = point(x1);
    double S, n;
    Eigen::Vector3d N;
    for (const Cell& c : m_cells) {
        if (!cell_at(p, c, S, N, n)) return false;
    }
    return true;
}
