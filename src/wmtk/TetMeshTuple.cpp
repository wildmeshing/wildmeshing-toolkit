...
bool TetMeshTuple::is_valid(const TetMesh& m) const
{
...
void TetMeshTuple::print_info() const
{
    logger().trace("tuple: {} {} {} {}", m_global_vid, m_local_eid, m_local_fid, m_global_tid);
}

void TetMeshTuple::print_info(const TetMesh& m) const
{
...
size_t TetMeshTuple::vid(const TetMesh&) const
{
    return m_global_vid;
} // update eid and fid
...
size_t TetMeshTuple::fid(const TetMesh& m) const
{
    ...
    for (int j = 0; j < 4; j++) {
        std::array<size_t, 3> tmp_v_ids = {
            {m.m_tet_connectivity[tid][utils::tet_element_topology::local_faces[j][0]],
             m.m_tet_connectivity[tid][utils::tet_element_topology::local_faces[j][1]],
             m.m_tet_connectivity[tid][utils::tet_element_topology::local_faces[j][2]]}};
...
size_t TetMeshTuple::tid(const TetMesh&) const
{
    return m_global_tid;
}
...
TetMeshTuple TetMeshTuple::switch_vertex(const TetMesh& m) const
{
    ...
    int l_vid2 = utils::tet_element_topology::local_edges[m_local_eid][1];
