...
TEST_CASE("sampler_bicubic_constant", "[displacement]")
{
    Image image(8, 8);
...
TEST_CASE("sampler_bicubic_linear", "[displacement]")
{
    WrappingMode wrapping_mode = WrappingMode::MIRROR_REPEAT;
    Image image(8, 8);
...
TEST_CASE("sampler_spline_constant", "[displacement]")
{
    Image image(8, 8);
    WrappingMode wrapping_mode = WrappingMode::MIRROR_REPEAT;
...
TEST_CASE("sampler_spline_linear", "[displacement]")
{
    WrappingMode wrapping_mode = WrappingMode::MIRROR_REPEAT;
    Image image(20, 20);
...
TEST_CASE("displacement_bicubic_edge_quadrature", "[.]")
{
    ...
    DisplacementPlane displ(image, SAMPLING_MODE::BICUBIC);
    for (int i = 0; i < 10; i++) {
    ...
    DisplacementPlane displ1(image, SAMPLING_MODE::BICUBIC);
    for (int i = 0; i < 10; i++) {
        Eigen::Vector2d p1, p2;
        p1 = Eigen::Vector2d(rand_dist(rand_generator), rand_dist(rand_generator));
        p2 = Eigen::Vector2d(rand_dist(rand_generator), rand_dist(rand_generator));
        auto edge_error = displ1.get_error_per_edge(p1, p2);
    ...
    DisplacementPlane displ2(image, SAMPLING_MODE::BICUBIC);
...
TEST_CASE("displacement_bicubic_area_quadrature")
{
    ...
    image.set(displacement_double, WrappingMode::MIRROR_REPEAT, WrappingMode::MIRROR_REPEAT);
    DisplacementPlane displ = DisplacementPlane(image, SAMPLING_MODE::BICUBIC);
    ...
    REQUIRE(abs(error2 + error1 - 0.179415250047267) < 1e-3);
}
