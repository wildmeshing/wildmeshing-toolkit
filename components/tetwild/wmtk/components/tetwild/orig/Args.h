...
struct Args
{
    ...
    // Also used to determine where to resize the scalar field (if a tet incident to a vertex has
    // larger energy than this threshold, then resize around this vertex).
    double filter_energy_thres = 10;

    // Threshold on the energy delta (avg and max) below which to rescale the target edge length
    // scalar field
