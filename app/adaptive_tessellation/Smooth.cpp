...
bool AdaptiveTessellationSmoothSeamVertexOperation::before(AdaptiveTessellation& m, const Tuple& t)
{
    ...
    m.write_obj(m.mesh_parameters.m_output_folder + fmt::format("/smooth_{:04d}.obj", cnt));
    m.write_obj_only_texture_coords(
        m.mesh_parameters.m_output_folder + fmt::format("/smooth_{:04d}_2d.obj", cnt));
