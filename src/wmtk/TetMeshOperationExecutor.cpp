...
TetMesh::TetMeshOperationExecutor::get_incident_tet_data(Tuple t)
{
    ...
    if (!simplex::utils::SimplexComparisons::equal(
            m_mesh,
            Simplex::vertex(t),
            Simplex::vertex(m_operating_tuple))) {
...
void TetMesh::TetMeshOperationExecutor::split_edge()
{
    ...
    m_new_vertex_ids.push_back(new_vids[0]);
    ...
    m_new_edge_ids.push_back(new_eids[0]);
    m_new_edge_ids.push_back(new_eids[1]);
    ...
    for (long i = 0; i < incident_faces.size(); ++i) {
        ...
        m_new_face_ids.push_back(new_fids[0]);
        m_new_face_ids.push_back(new_fids[1]);
        m_new_edge_ids.push_back(splitting_eids[0]);
    ...
    for (long i = 0; i < incident_tets.size(); ++i) {
        ...
        m_new_tet_ids.push_back(new_tids[0]);
        m_new_tet_ids.push_back(new_tids[1]);
        m_new_face_ids.push_back(split_fids[0]);
