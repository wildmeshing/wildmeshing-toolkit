...
void OperationSettings<tri_mesh::EdgeSwapSafe>::initialize_invariants(const TriMesh& m)
{
...
namespace tri_mesh {
EdgeSwapSafe::EdgeSwapSafe(Mesh& m, const Tuple& t, const OperationSettings<EdgeSwapSafe>& settings)
    : TriMeshOperation(m)
    , TupleOperation(settings.invariants, t)
    , m_settings{settings}
{}

std::string EdgeSwapSafe::name() const
{
    return "tri_mesh_edge_swap_base";
}

Tuple EdgeSwapSafe::return_tuple() const
{
    return m_output_tuple;
}

bool EdgeSwapSafe::execute()
{
    // input
    //    / \ .
    //   /   \ .
    ...
    Tuple split_ret;
    {
        tri_mesh::EdgeSplitAtMidpoint split_op(mesh(), input_tuple(), m_settings.split_settings);
        if (!split_op()) {
            return false;
        }
    ...
    Tuple coll_ret;
    {
        tri_mesh::EdgeCollapseToMidpoint coll_op(
            mesh(),
            coll_input_tuple,
