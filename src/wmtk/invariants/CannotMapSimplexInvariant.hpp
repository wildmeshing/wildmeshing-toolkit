...
/**
 * This invariant returns true if the given simplex, or one of its neighbors can be mapped to a child mesh.
 */
class CannotMapSimplexInvariant : public Invariant
{
public:
    CannotMapSimplexInvariant(const Mesh& parent_mesh, const Mesh& child_mesh);
    CannotMapSimplexInvariant(const Mesh& parent_mesh, const Mesh& child_mesh, PrimitiveType mapped_simplex_type);
