...
def _name_to_internal_tag(mesh_path: str) -> dict:
    """For a wmtk-compatible `_groups.msh`, return {phys_group_name: int}.
...
def _ensure_msh_extension(path: str) -> str:
    """Ensure the path ends with .msh (or .msh.gz)."""
    p = Path(path)
    if p.suffix != ".msh":
        if p.suffix != "":
            raise ValueError(
...
def strip_envelope(msh_path, output_path=None):
    """Remove the WMTK envelope physical group from a `_groups.msh`.
    ...
    Parameters:
    - msh_path: input `_groups.msh` path.
...
if __name__ == "__main__":
    ...
    if args.d2:
        ...
        # smooth_cfg = {
        #     "polyfem": "/Users/uday/Research/image_simulation/polyfem/build/PolyFEM_bin",
