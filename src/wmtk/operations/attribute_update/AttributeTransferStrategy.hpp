...
// {
//     auto acc1 =
//         m_parent_handle1.mesh().create_const_accessor(m_parent_handle1.template
//         as<ParentType1>());
//     auto simps =
//         AttributeTransferStrategyBase::get_parent_simplices(handle(), m_parent_handle1,
//         my_simplex);
...
//     auto acc2 =
//         m_parent_handle2.mesh().create_const_accessor(m_parent_handle2.template
//         as<ParentType2>());
...
//     using Index = Eigen::Index;
//     for (Index j = 0; j < Index(simps.size()); ++j) {
//         simplex::Simplex s = simplex::Simplex(my_simplex.primitive_type(), simps[j]);
//         if (s == my_simplex) {
//             Tuple t = m_parent_handle1.mesh().switch_vertex(simps[j]);
//             A.col(j) = acc1.const_vector_attribute(t);
//             B.col(j) = acc2.const_vector_attribute(t);
//         } else {
...
//         std::runtime_error(
//             "SingleAttributeTransferStrategyWithMultiParents: parent handle's types are
//             different");
//     }
