    ...
    const std::vector<bool>& tet_face_on_input_surface)
{
    ...
    for (int i = 0; i < vert_capacity(); i++) {
        VA[i].m_pos = v_rational[i];
        VA[i].m_posf = to_double(v_rational[i]);
    ...
    logger().info("Init from VolumeRemesher #F = {}", faces.size());
    for (const Tuple& f : faces) {
        const size_t fid = f.fid(*this);
        if (m_face_attribute[fid].m_is_surface_fs) {
            const auto vs = get_face_vids(f);
            VA[vs[0]].m_is_on_surface = true;
            VA[vs[1]].m_is_on_surface = true;
            VA[vs[2]].m_is_on_surface = true;
        }
    }

    // track bounding box
    for (size_t i = 0; i < faces.size(); i++) {
        const auto vids = get_face_vids(faces[i]);
        int on_bbox = -1;
        for (int k = 0; k < 3; k++) {
            if (VA[vids[0]].m_pos[k] == m_params.box_min[k] &&
                VA[vids[1]].m_pos[k] == m_params.box_min[k] &&
                VA[vids[2]].m_pos[k] == m_params.box_min[k]) {
                on_bbox = k * 2;
                break;
            }
            if (VA[vids[0]].m_pos[k] == m_params.box_max[k] &&
                VA[vids[1]].m_pos[k] == m_params.box_max[k] &&
                VA[vids[2]].m_pos[k] == m_params.box_max[k]) {
                on_bbox = k * 2 + 1;
                break;
            }
        }
        ...
        for (size_t vid : vids) {
            VA[vid].on_bbox_faces.push_back(on_bbox);
        }
    }
