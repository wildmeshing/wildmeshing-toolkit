...
TEST_CASE("smoothing_Newton_Method")
{
    ...
    std::shared_ptr<function::AMIPS2D> per_tri_amips = std::make_shared<function::AMIPS2D>(
        mesh,
        mesh.get_attribute_handle<double>("position", PrimitiveType::Vertex));
    op_settings.energy =
        std::make_unique<function::LocalDifferentiableFunction>(per_tri_amips, PrimitiveType::Face);
    ...
    spdlog::warn("Initial valuenorm: {}", factory.settings().energy->get_value(tuple));
    spdlog::warn(
        "Initial gradient: norm: {}",
        factory.settings().energy->get_gradient(tuple).norm());
    while (factory.settings().energy->get_gradient(tuple).norm() > 1e-10) {
        scheduler.run_operation_on_all(PrimitiveType::Vertex, "optimize_vertices");
        REQUIRE(scheduler.number_of_successful_operations() > 0);
        tuple = mesh.face_tuple_from_vids(0, 1, 1);
    }
