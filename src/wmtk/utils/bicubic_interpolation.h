...
inline float get_value(float x)
{
    return x;
}
inline float get_value(double x)
{
    return static_cast<float>(x);
}
inline float get_value(DScalar2<double, Eigen::Vector2d, Eigen::Matrix2d> x)
{
    return static_cast<float>(x.getValue());
}
...
eval_bicubic_coeffs(const wmtk::BicubicVector<float>& coeffs, const T& sx, const T& sy)
{
    ...
    const auto xx = sx - floor(get_value(sx));
    const auto yy = sy - floor(get_value(sy));
