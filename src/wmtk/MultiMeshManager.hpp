...
public:
    ...
    static void write_tuple_map_attribute(
        MeshAttributeHandle<long> map_handle,
        Mesh& source_mesh,
        const Tuple& source_tuple,
        const Tuple& target_tuple);
    static std::tuple<Tuple, Tuple> read_tuple_map_attribute(
        MeshAttributeHandle<long> map_handle,
    ...
    // size of child_meshes.size(), store the map to the base_tuple of the child_meshes (on the top
    // level simplex of each child_mesh) a map is a pair of two tuples, from a tuple in parent_mesh
    // to a tuple in child_mesh i.e. Dim(map_to_child[i]) == Dim(child_meshes[i])
    ...
    static void register_child_mesh(
        Mesh& parent_mesh,
    ...
    static std::vector<Simplex> find_all_simplices_in_child_mesh(
    ...
    static std::vector<Tuple> map_edge_tuple_to_all_children(
        const Mesh& parent_mesh,
