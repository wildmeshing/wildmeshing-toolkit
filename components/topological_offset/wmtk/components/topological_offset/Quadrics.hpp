 ...
 * attributes is left to the caller (see quadric_move_front_vertex() in FrontSmooth3d.cpp)
 * instead of being built into this class.
 */
class Quadrics
{
public:
     ...
     * feature edges: LOWER keeps more singular values, so more directions are constrained by the
     * planes and the result is pulled more strongly onto a sharp feature; HIGHER discards more,
     * leaving those directions at p. See Eigen::JacobiSVD::setThreshold().
     *
     * (The doc this replaced called a lower threshold "less sensitive" in the same breath as
     * "pulling the result more strongly toward sharp features", which are opposite claims. The
     * code forwards the value straight to setThreshold(), so the second half is the true one.)
     */
