...
CloughTocherPatch::CloughTocherPatch(Eigen::Matrix<double, 12, 3>& boundary_data)
...
void CloughTocherPatch::set_lagrange_nodes(
...
int CloughTocherPatch::triangle_ind(const double& u, const double& v, const double& w) const
{
    int idx = -1;
    for (int i = 0; i < 3; ++i) {
...
CloughTocherPatch::monomial_basis_eval(const double& u, const double& v, const double& w) const
{
...
PlanarPoint normalize_domain_point(const ConvexPolygon& domain, const PlanarPoint& domain_point)
{
...
CloughTocherPatch::CT_eval_normalized(int idx, const double& u, const double& v) const
{
    const double w = 1.0 - u - v;
    // std::cout << "subtri_idx: " << idx << std::endl;
    Eigen::Matrix<double, 10, 1> bb_vector = CloughTocherPatch::monomial_basis_eval(u, v, w);
...
Eigen::Matrix<double, 3, 1> CloughTocherPatch::CT_eval(const double& u, const double& v) const
{
    const double w = 1.0 - u - v;
...
std::array<Eigen::Matrix<double, 10, 3>, 3> CloughTocherPatch::get_coeffs() const
{
    return m_CT_coeffs;
}
    ...
    Eigen::Matrix<double, 12, 1>& external_boundary_data) const
{
    ...
    double value = (m_CT_matrices[idx] * external_boundary_data).transpose() * bb_vector;

    return value;
}

void CloughTocherPatch::triangulate(
    ...
    std::array<Eigen::MatrixXi, 3>& F) const
{
    for (int n = 0; n < 3; ++n) {
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            boundary_segments_coeffs[i][1] = m_CTtri_bounds[n](i, 0) - m_CTtri_bounds[n](i, 2);
            boundary_segments_coeffs[i][2] = m_CTtri_bounds[n](i, 1) - m_CTtri_bounds[n](i, 2);
        }
...
void CloughTocherPatch::triangulate_normalized(
    size_t num_refinements,
    std::array<Eigen::MatrixXd, 3>& V,
    std::array<Eigen::MatrixXi, 3>& F) const
{
...
void CloughTocherPatch::parametrize_patch_boundaries(
...
void CloughTocherPatch::view() const
{
    ...
    polyscope::init();
    for (int i = 0; i < 3; ++i) {
        polyscope::registerSurfaceMesh("patch " + std::to_string(i), V[i], F[i])->setEdgeWidth(0);
        polyscope::registerPointCloud("control points" + std::to_string(i), m_CT_coeffs[i]);
    }
