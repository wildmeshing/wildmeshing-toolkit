...
void link_vertex_vertex(
    ...
    std::vector<IdSimplex>& collection)
{
    ...
    do {
        ...
        collection.emplace_back(mesh.get_id_simplex(t_collect, PV));
    ...
    collection.emplace_back(mesh.get_id_simplex(mesh.switch_tuple(t, PV), PV));
    if (mesh.is_boundary_edge(t)) {
        return;
    }
    t = mesh.switch_tuples(t, {PF, PE});
    ...
    std::vector<IdSimplex>& collection)
{
    ...
    do {
        const Tuple t_collect = mesh.switch_tuples(t, {PV, PE});
        collection.emplace_back(mesh.get_id_simplex(t_collect, PE));
    ...
    std::vector<IdSimplex>& collection)
{
    ...
    collection.emplace_back(mesh.get_id_simplex(mesh.switch_tuples(t, {PE, PV}), PV));
    if (!mesh.is_boundary_edge(t)) {
        collection.emplace_back(mesh.get_id_simplex(mesh.switch_tuples(t, {PF, PE, PV}), PV));
    }
}
} // namespace
...
SimplexCollection link_single_dimension(
    ...
    const bool sort_and_clean)
{
    std::vector<IdSimplex> link_simplices;
    ...
    case PrimitiveType::Edge:
        if (link_type == PrimitiveType::Vertex) {
            ...
            for (Tuple t : cell_tuples) {
                ...
                all_cofaces.emplace_back(mesh.get_id_simplex(t, PV));
                all_cofaces.emplace_back(mesh.get_id_simplex(mesh.switch_vertex(t), PV));
            }
        }
    ...
    const bool sort_and_clean)
{
    ...
    return SimplexCollection(mesh, collection.id_simplex_vector(link_type));
}
