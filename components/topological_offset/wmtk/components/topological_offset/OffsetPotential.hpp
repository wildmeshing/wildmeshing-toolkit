...
/**
 * @brief AMIPS of REAL 3D triangles, minimised over a 2D search space: a plane through one corner.
 *
 *     E(x) = w * sum over cells of  ( |p-a|^2 + |p-b|^2 + |a-b|^2 ) / (2 sqrt(3) * area(p,a,b))
 *     with p(x) = p0 + x0 * t1 + x1 * t2
 *
 * The moving vertex p is confined to the plane spanned by the orthonormal t1, t2; a and b are the
 * triangle's two other corners, at their true 3D positions, and never move. The number scored is
 * the triangle's ACTUAL shape: the numerator is its three real squared edge lengths and the
 * denominator its real area, both measured in 3D. Nothing is flattened.
 *
 * THIS IS wmtk::AMIPS2D_energy, WRITTEN INTRINSICALLY. That kernel's closed form reduces exactly
 * to (l01^2 + l12^2 + l02^2) / (2 sqrt(3) * signed area) -- both of its halves are similarity
 * invariants of the triangle -- so a triangle laid flat in its own plane and scored by AMIPS2D
 * gives this same number, with 2 at the equilateral and +infinity as it degenerates. The reason
 * for a separate class is the DOMAIN, not the measure: AMIPSEnergy2D's cells are six plane
 * coordinates, so reaching it at all means first projecting the one-ring into the plane, and a
 * face tilted out of the plane is then scored on a shape it does not have. Over a rounded feature
 * of a euclidean level set that tilt is about h / (2 delta) for an edge of length h, i.e. roughly
 * 30 degrees at h = delta, which is not a small correction to a shape measure.
 *
 * ORIENTATION AND THE BARRIER. area() here is unsigned, so the energy alone cannot tell a folded
 * triangle from a good one, and its +infinity at zero area is a two-sided spike a line search
 * could step across. Each cell therefore carries the outward side of the surface, n_out: a
 * configuration is valid only while (a - p) x (b - p) still points to that side of it, which the
 * caller arranges to hold at x = 0. value() is NaN and is_step_valid() false outside that,
 * exactly as AMIPSEnergy2D uses orient2d, so the barrier the solver sees is one-sided.
 *
 * Both S = sum of squared edge lengths and N = (a - p) x (b - p) are polynomial in p -- S
 * quadratic, N affine -- so the gradient and Hessian in x are the exact chain through
 * d(S / (sqrt(3) |N|)), projected by [t1 t2].
 */
class TangentAMIPSEnergy : public polysolve::nonlinear::Problem
{
public:
    using typename polysolve::nonlinear::Problem::Scalar;
    using typename polysolve::nonlinear::Problem::THessian;
    ...
    struct Cell
    {
        Eigen::Vector3d a, b; ///< the two fixed corners, world positions, wound so that
        ///< ((a - p) x (b - p)) . n_out > 0 at x = 0
    };
    /// p0 is the plane's origin and the vertex's current position, so x = 0 is the start. t1, t2
    /// must be orthonormal; n_out is the surface's outward side, used only by the validity test.
    TangentAMIPSEnergy(
    ...
    /// The 3D point the plane coordinates x stand for.
    Eigen::Vector3d point(const TVector& x) const { return m_p0 + x(0) * m_t1 + x(1) * m_t2; }

private:
    /// One cell at p: S, N = (a - p) x (b - p) and n = |N|. false when the triangle has folded
    /// past n_out or has no area, which is where the energy is not defined.
    bool cell_at(const Eigen::Vector3d& p, const Cell& c, double& S, Eigen::Vector3d& N, double& n)
        const;
    std::vector<Cell> m_cells;
    Eigen::Vector3d m_p0, m_t1, m_t2, m_n_out;
    double m_weight;
};
