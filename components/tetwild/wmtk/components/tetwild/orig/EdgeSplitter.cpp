    ...
    Eigen::MatrixXi& F)
{
    ...
    for (unsigned j = 0; j < tets.size(); ++j) {
        for (int k = 0; k < 4; k++) {
            for (int r = 0; r < 3; r++) V(i * 4 + k, r) = tet_vertices[tets[j][k]].posf[r];
        }
...
void EdgeSplitter::split()
{
    ...
    logger().debug("Queue size = {}", es_queue.size());
    ...
    while (!es_queue.empty()) {
        ...
        //);
        es_queue.pop();
        if (splitAnEdge(v_ids)) suc_counter++;
        counter++;
    ...
    // maybe, we can marked all the splitted tets in this pass and the update their quality in the
    // end? -- Try it. todo ...
...
bool EdgeSplitter::splitAnEdge(const std::array<int, 2>& edge)
{
    ...
    if (!is_found) v_empty_start = v_is_removed.size();
    ...
    // check is_valid
    tet_vertices[v_id].adaptive_scale =
    ...
    tet_vertices[v_id].posf = (tet_vertices[v1_id].posf + tet_vertices[v2_id].posf) / 2;
    ...
    if (isFlip(new_tets)) {
        tet_vertices[v_id].pos = (tet_vertices[v1_id].pos + tet_vertices[v2_id].pos) / 2;
        tet_vertices[v_id].posf = to_double(tet_vertices[v_id].pos);
    ...
    for (int i = 0; i < new_t_ids.size(); i++) {
        for (int j = 0; j < 4; j++) { // v1->v
            if (tets[new_t_ids[i]][j] == v2_id) is_surface_fs[new_t_ids[i]][j] = state.NOT_SURFACE;
    ...
    for (int i = 0; i < old_t_ids.size(); i++) {
        for (int j = 0; j < 4; j++) { // v2->v
            if (tets[old_t_ids[i]][j] == v1_id) is_surface_fs[old_t_ids[i]][j] = state.NOT_SURFACE;
        }
    }
...
{
    if (is_check_quality) return true;
...
bool EdgeSplitter::isSplittable_cd1(int v1_id, int v2_id, double weight)
{
    double adaptive_scale =
        (tet_vertices[v1_id].adaptive_scale + tet_vertices[v2_id].adaptive_scale) / 2.0;
    //    if(adaptive_scale==0){
...
void EdgeSplitter::getNewTetSlots(int n, std::vector<int>& new_conn_tets)
{
    ...
    if (cnt < n) {
        for (int i = 0; i < n - cnt; i++) new_conn_tets.push_back(tets.size() + i);
