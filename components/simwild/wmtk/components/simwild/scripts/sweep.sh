#!/usr/bin/env bash
# Launch / control the simwild Thingi10K sweep on kirby, inside tmux so it survives
...
ROOT=${SIMWILD_ROOT:-/u/3/daniele/simwild-thingi10k-sweep}
export SIMWILD_ROOT="$ROOT"
RUNNER="$ROOT/scripts/run_simwild_sweep.py"
SESSION=simwild-sweep

export SIMWILD_PARALLEL=${SIMWILD_PARALLEL:-16}
export SIMWILD_THREADS=${SIMWILD_THREADS:-8}               # 16 x 8 = 128 = kirby's core count
export SIMWILD_JOB_TIMEOUT=${SIMWILD_JOB_TIMEOUT:-10800}   # 3 hours
...
# model is genuinely huge -- see tetwild's sweep.sh for the Thingi10K 338910 case.
export SIMWILD_MEM_GB=${SIMWILD_MEM_GB:-128}               # per model
...
start() {
    ...
    local env_extra=""
    if [ "$mode" = quick ]; then
        env_extra="SIMWILD_LIMIT=100 SIMWILD_SAMPLE=spread"
    else
        env_extra="SIMWILD_SAMPLE=smallest"
    fi
    ...
    tmux new-session -d -s "$SESSION" -n "$mode" \
        "SIMWILD_OUT='$out' SIMWILD_PARALLEL=$SIMWILD_PARALLEL SIMWILD_THREADS=$SIMWILD_THREADS \
         SIMWILD_JOB_TIMEOUT=$SIMWILD_JOB_TIMEOUT SIMWILD_MEM_GB=$SIMWILD_MEM_GB \
...
case "${1:-}" in
    ...
    stop)
        for m in quick full; do
            out="$(out_for "$m")"
            [ -f "$out/sweep.pid" ] && SIMWILD_OUT="$out" python3 "$RUNNER" stop
        done
        ;;
    attach) tmux attach -t "$SESSION" ;;
    report)
        out="$(out_for "${2:-full}")"
        SIMWILD_OUT="$out" SIMWILD_REPORT_ONLY=1 python3 "$RUNNER"
        ;;
    status)
        for m in quick full; do
            out="$(out_for "$m")"
