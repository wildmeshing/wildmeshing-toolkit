    ...
    PrimitiveType type)
{
    ...
    acc_pos.vector_attribute(vertex_tuple) = target_p;
    if (is_invert(mesh, pos_handle, vertex_tuple, type)) {
        for (int i = 0; i < 8; ++i) {
            if (is_invert(mesh, pos_handle, vertex_tuple, type)) {
                target_p = acc_pos.vector_attribute(vertex_tuple);
                acc_pos.vector_attribute(vertex_tuple) = (last_best_p + target_p) * 0.5;
            } else {
                last_best_p = acc_pos.vector_attribute(vertex_tuple);
                acc_pos.vector_attribute(vertex_tuple) = (last_best_p + target_p) * 0.5;
            }
        }
        acc_pos.vector_attribute(vertex_tuple) = last_best_p;
        if (is_invert(mesh, pos_handle, vertex_tuple, type)) {
            acc_pos.vector_attribute(vertex_tuple) = original_p;
        }
    }
}
