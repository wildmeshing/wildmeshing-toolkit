...
JOB_TIMEOUT = int(os.environ.get("TETWILD_JOB_TIMEOUT", "10800"))  # seconds, 3h
MEM_GB = int(os.environ.get("TETWILD_MEM_GB", "128"))  # 0 = no cap
LIMIT = int(os.environ.get("TETWILD_LIMIT", "0"))  # 0 = no limit
...
PHASES_RE = re.compile(
    r"TETWILD_PHASES threads (\S+) load (\S+)s simplify (\S+)s insertion (\S+)s "
    r"optimization (\S+)s finalize (\S+)s output (\S+)s")
...
def generate_html_report(rows, stats, report_dir, out_dir, dataset_dir=None,
    ...
    h.append("<title>tetwild Thingi10K sweep report</title>")
    h.append("<style>{}</style></head><body><div id='tip'></div><div class='wrap'>".format(_CSS))

    h.append("<h1>tetwild &mdash; Thingi10K sweep</h1>")
    ...
    h.append("<footer>tetwild / Thingi10K &middot; {}</footer>".format(
...
def generate_report():
    ...
    lines = []
    lines.append("=" * 64)
    lines.append("tetwild / Thingi10K batch report")
    lines.append("=" * 64)
    ...
    html_path = generate_html_report(
        ...
        {"generated": datetime.datetime.now().strftime("%Y-%m-%d %H:%M"),
         "banner": os.environ.get("TETWILD_REPORT_BANNER", "")},
        REPORT_DIR,
        OUT_DIR,
        DATASET_DIR,
        MESH_EXTENSIONS,
...
def run_sweep():
    ...
    if not _memory_capped_available():
        sys.exit(f"cannot create a systemd scope with MemoryMax={MEM_GB}G -- the user "
                 f"manager is not usable here. Set TETWILD_MEM_GB=0 to run uncapped.")
