...
//         };
//         if (!check_inter(std::get<0>(lk0), std::get<0>(lk1), std::get<0>(lk_01),
//         std::get<0>(lk_i)))
//             return false;
//         if (!check_inter(std::get<1>(lk0), std::get<1>(lk1), std::get<1>(lk_01),
//         std::get<1>(lk_i)))
//             return false;
//         if (!check_inter(std::get<2>(lk0), std::get<2>(lk1), std::get<2>(lk_01),
//         std::get<2>(lk_i)))
//             return false;
...
bool TetMesh::link_condition(const Tuple& t)
{
    using EdgeT = std::array<size_t, 2>;
    using FaceT = std::array<size_t, 3>;
    using TetT = std::array<size_t, 4>;

    const size_t v0 = t.vid(*this);
    const size_t v1 = t.switch_vertex(*this).vid(*this);

    auto& VC = m_vertex_connectivity;
    auto& TC = m_tet_connectivity;
    ...
    /**
     * Compute the link of a given simplex
     * Return a tuple of sets for 1. vertices, 2. edges, and 3. faces.
     */
    auto link = [&intersects, &conn = TC](
                    const auto& verts,
                    const std::vector<size_t>& tets,
                    const std::vector<FaceT>& faces) {
        auto conn_verts = std::set<size_t>();
        auto conn_edges = std::set<EdgeT>();
        auto conn_faces = std::set<FaceT>();
        logger().trace("In Link {}", verts);
        auto collect = [&](const TetT& tet) {
            for (auto j = 0; j < 4; j++) {
                const size_t& vj = tet[j];
                if (intersects(std::array<size_t, 1>{{vj}}, verts)) {
                    continue;
                }
                conn_verts.insert(vj);
            }
            for (const auto& e : utils::tet_element_topology::local_edges) {
                auto e0 = tet[e[0]], e1 = tet[e[1]];
                EdgeT edge{{std::min(e0, e1), std::max(e0, e1)}};
                if (intersects(edge, verts)) {
                    continue;
                }
                conn_edges.emplace(edge);
            }
            for (const auto& f : utils::tet_element_topology::local_faces) {
                FaceT face{{tet[f[0]], tet[f[1]], tet[f[2]]}};
                if (intersects(face, verts)) {
                    continue;
                }
                std::sort(face.begin(), face.end());
        ...
        for (const size_t t : tets) {
            collect(conn[t].m_indices);
        }
        constexpr size_t dummy = std::numeric_limits<size_t>::max();
        logger().trace("in face {}", faces);
        for (auto& f : faces) {
            auto tet = TetT{{f[0], f[1], f[2], dummy}};
            collect(tet);
        }
        logger().trace("out points {}", conn_verts);
        return std::tuple<std::set<size_t>, std::set<EdgeT>, std::set<FaceT>>{
            conn_verts,
            conn_edges,
            conn_faces};
    };
    const std::vector<size_t>& closure0 = VC[v0].m_conn_tets;
    const std::vector<size_t>& closure1 = VC[v1].m_conn_tets;
    const std::vector<FaceT> bnd0 = vertex_adjacent_boundary_faces(this->tuple_from_vertex(v0));
    auto bnd1 = vertex_adjacent_boundary_faces(this->tuple_from_vertex(v1));
    auto lk0 = link(std::array<size_t, 1>{{v0}}, closure0, bnd0);
    auto lk1 = link(std::array<size_t, 1>{{v1}}, closure1, bnd1);
    ...
    // link of edge
    auto common_tets = set_intersection(closure0, closure1);
    auto lk_01 = link(EdgeT{{v0, v1}}, common_tets, bnd01);
    if (!std::get<2>(lk_01).empty()) {
        return false;
    }

    auto lk_i = std::tuple<std::vector<size_t>, std::vector<EdgeT>, std::vector<FaceT>>();
    auto check_inter = [](const auto& l0, const auto& l1, const auto& l01, auto& l_i) {
        // link(v0) intersect link(v1) == link(edge 01)
        logger().trace(">> Checking l0 {} l1 {}, l01 {}", l0, l1, l01);
        std::set_intersection(l0.begin(), l0.end(), l1.begin(), l1.end(), std::back_inserter(l_i));
        return (
            l_i.size() == l01.size() && std::equal(l_i.begin(), l_i.end(), l01.begin(), l01.end()));
    };
...
void TetMesh::collapse_edge_rollback(
    size_t& v1_id,
    std::map<size_t, wmtk::TetMesh::VertexConnectivity>& rollback_vert_conn,
    std::vector<size_t>& n1_t_ids,
    std::vector<size_t>& new_tet_id,
