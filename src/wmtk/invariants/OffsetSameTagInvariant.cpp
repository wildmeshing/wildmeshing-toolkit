#include "OffsetSameTagInvariant.hpp"
#include <wmtk/Mesh.hpp>

namespace wmtk {
OffsetSameTagInvariant::OffsetSameTagInvariant(
    const Mesh& m,
...
{}
bool OffsetSameTagInvariant::before(const Tuple& t) const
{
    ConstAccessor<long> vertex_tag_accessor = mesh().create_accessor(m_vertex_tag_handle);
    ConstAccessor<long> edge_tag_accessor = mesh().create_accessor(m_edge_tag_handle);
    ...
    if (vt0 == m_input_tag_value && vt1 == m_input_tag_value) {
        long et = edge_tag_accessor.const_vector_attribute(t)(0);
        return et == m_embedding_tag_value;
    }
    return false;
}
} // namespace wmtk
