...
std::vector<Simplex> TetEdgeSwap::execute(const Simplex& simplex)
{
    ...
    assert(edges_generated_by_split.size() >= simplices_deleted_by_collapse[1].size());

    bool able_to_return_edges = true;
    if (edges_generated_by_split.size() == simplices_deleted_by_collapse[1].size())
    ...
    if (able_to_return_edges) {
        for (const auto& e_split : edges_generated_by_split) {
            ...
            if (preserve_flag) {
                edge_tuples_generated_by_swap.push_back(e_split);
            }
        }

        assert(
            edge_tuples_generated_by_swap.size() ==
            (edges_generated_by_split.size() - simplices_deleted_by_collapse[1].size()));
    } else {
        // return face
        auto faces_generated_by_split = m_split.new_face_tuples();
        assert(faces_generated_by_split.size() >= simplices_deleted_by_collapse[2].size());

        for (const auto& f_split : faces_generated_by_split) {
            bool preserve_flag = true;
            for (const auto& f_collapse : simplices_deleted_by_collapse[2]) {
                if (simplex::utils::SimplexComparisons::equal(
                        mesh(),
                        simplex::Simplex::face(f_split),
                        f_collapse)) {
                    preserve_flag = false;
                    break;
                }
            ...
            if (preserve_flag) {
                face_tuples_generated_by_swap.push_back(f_split);
            }
        }

        assert(
            face_tuples_generated_by_swap.size() ==
            (faces_generated_by_split.size() - simplices_deleted_by_collapse[2].size()));
    }
    ...
    if (able_to_return_edges) {
        ...
        for (long i = 0; i < edge_tuples_generated_by_swap.size(); ++i) {
            edges_generated_by_swap.emplace_back(Simplex::edge(edge_tuples_generated_by_swap[i]));
        }
        ...
        for (long i = 0; i < face_tuples_generated_by_swap.size(); ++i) {
            faces_generated_by_swap.emplace_back(Simplex::face(face_tuples_generated_by_swap[i]));
        }
