...
#include <wmtk/utils/SamplingEnvelope.hpp>
...
TEST_CASE("edge_collapsing", "[tetwild_operation]")
{
    ...
    REQUIRE(tetwild.check_mesh_connectivity_validity());
    CHECK(tetwild.get_vertices().size() == 1037);
...
TEST_CASE("sample_envelope", "[tetwild_operation]")
{
    ...
    SamplingEnvelope envelope(1e-1);
    envelope.init(V, F, 1e-3);

    for (int i = 0; i < F.rows(); ++i) {
        std::array<Eigen::Vector3d, 3> tri = {{V.row(F(i, 0)), V.row(F(i, 1)), V.row(F(i, 2))}};
        REQUIRE_FALSE(envelope.is_outside(tri));
    }
}