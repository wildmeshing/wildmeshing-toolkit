...
namespace wmtk
{
    ...
    public:
        ...
        public:
            ...
            size_t eid() const {//todo: discuss
                return m_tid * 6 + m_eid;
            }
            size_t fid() const {//todo: discuss: if output same global fid for the two sides(tets) of face, how to give the two sides different value?
            ...
            Tuple switch_face(const TetMesh &m) const {
                ...
                for (int j = 0; j < 4; j++) {
                    ...
                    int cnt = 0;
                    for (int k = 0; k < 3; k++) {
                        if (m_local_faces[j][k] == l_v1_id || m_local_faces[j][k] == l_v2_id)
                            cnt++;
                        if (cnt == 2) {
                            loc.m_fid = j;
                            return loc;
            ...
            Tuple switch_tetrahedron(const TetMesh &m) const {
                ...
                auto tmp = set_intersection(m.m_vertex_connectivity[v1_id].m_conn_tets,
