...
TEST_CASE("test_extract_child_mesh", "[multimesh][2D]")
{
    SECTION("single_triangle")
    {
        ...
        REQUIRE(child.get_all(PE).size() == 3);
        REQUIRE(child.get_all(PV).size() == 3);
    }
    SECTION("two_neighbors")
    {
        ...
        REQUIRE(child0.get_all(PE).size() == 3);
        REQUIRE(child0.get_all(PV).size() == 3);
        REQUIRE(child1.get_all(PE).size() == 2);
        REQUIRE(child1.get_all(PV).size() == 3);
        REQUIRE(child2.get_all(PE).size() == 2);
        REQUIRE(child2.get_all(PV).size() == 3);
    }
}
...
//                 std::tuple<Tuple, Tuple>{Tuple(), Tuple()},
//                 std::tuple<Tuple, Tuple>{parent.tuple_from_id(PF, 2), child0.tuple_from_id(PF,
//                 0)}};

//             std::vector<std::tuple<Tuple, Tuple>> p_to_c1_map{
//                 std::tuple<Tuple, Tuple>{parent.tuple_from_id(PF, 0), child1.tuple_from_id(PF,
//                 0)}, std::tuple<Tuple, Tuple>{parent.tuple_from_id(PF, 1),
//                 child1.tuple_from_id(PF, 1)}, std::tuple<Tuple, Tuple>{Tuple(), Tuple()},
//             };

//             std::vector<std::tuple<Tuple, Tuple>> c0_to_p_map{
//                 std::tuple<Tuple, Tuple>{child0.tuple_from_id(PF, 0), parent.tuple_from_id(PF,
//                 2)}};

//             std::vector<std::tuple<Tuple, Tuple>> c1_to_p_map{
//                 std::tuple<Tuple, Tuple>{child1.tuple_from_id(PF, 0), parent.tuple_from_id(PF,
//                 0)}, std::tuple<Tuple, Tuple>{child1.tuple_from_id(PF, 1),
