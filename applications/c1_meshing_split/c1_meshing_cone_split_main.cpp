...
#include "c1_meshing_cone_split_spec.hpp"
    ...
    for (int64_t i = 0; i < cone_edges.size(); ++i) {
        ...
        for (const auto& e : surface_mesh->get_all(PrimitiveType::Edge)) {
            ...
            if ((ev0 == v0 && ev1 == v1) || (ev0 == v1 && ev1 == v0)) {
                ...
                // at most one
                for (int64_t j = i + 1; j < cone_edges.size(); ++j) {
                    if (cone_edges[j][0] == v1 && cone_edges[j][1] == v0) {
                        // update v0 to v_new
                        cone_edges[j][1] = vid_accessor.scalar_attribute(new_v_tuple);
                        break;
                    }
                }
