...
ClosedStarIterable::Iterator& ClosedStarIterable::Iterator::operator++()
{
    ...
    if (m_phase == IteratorPhase::Link) {
        if (m_pt < m - s - 1 && !m_it.is_intermediate()) {
            ...
            if (m - s - 1 == 0 && m_sub_face_counter < 2) {
...
void ClosedStarIterable::Iterator::step_faces()
{
    ...
    case PT: {
        log_and_throw_error("not tested");
        // use look-up table to get all the faces
        ++m_sub_face_counter;
        switch (m_sub_face_counter) {
        case 1: m_sub_pt = 1; return;
        case 2: m_sub_pt = 2; return;
        case 3:
            m_sub_pt = 1;
        ...
        case 4:
            m_sub_pt = 0;
            m_t = mesh.switch_tuples(m_t, {PE, PV, PE, PF, PE});
            return;
        ...
        case 8:
            ...
            m_t = mesh.switch_tuples(m_t, {PE, PV, PE, PF, PE});
            return;
        case 9: m_sub_pt = 1; return;
        case 10: m_sub_pt = 2; return;
        case 11:
            m_sub_pt = 1;
            m_t = mesh.switch_tuple(m_t, PE);
            return;
