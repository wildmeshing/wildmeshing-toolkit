    ...
    Eigen::MatrixXd& val)
{
    ...
    p_n_nodes_3d(p, val);
    int i = round(val(local_index, 0) * p), j = round(val(local_index, 1) * p),
        k = round(val(local_index, 2) * p);
    ...
    Eigen::MatrixXd& val)
{
    ...
    ijk = convert_local_index_to_ijk(local_index, p);
    val.resize(uv.rows(), uv.cols());
    val.col(0) = P(ijk(1), p, y) * (P_prime(ijk(0), p, x) * P(ijk(2), p, 1 - x - y) -
                                    P(ijk(0), p, x) * P_prime(ijk(2), p, 1 - x - y));
    val.col(1) = P(ijk(0), p, x) * (P_prime(ijk(1), p, y) * P(ijk(2), p, 1 - x - y) -
                                    P(ijk(1), p, y) * P_prime(ijk(2), p, 1 - x - y));
}
