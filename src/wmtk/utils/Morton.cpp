...
{
    data =
        (SplitBy3Bits21(x) | SplitBy3Bits21(y) << 1 | SplitBy3Bits21(z) << 2) | 0x7000000000000000;
}
...
MortonCode64 MortonCode64::InvertDimension() const
{
    ...
    uint64_t sum =
        (c | ~mask) +
        1; // fill the bits that are not in the mask with 1 to make sure carry gets carried on
