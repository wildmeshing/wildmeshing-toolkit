...
TriAMIPS::TriAMIPS(const TriMesh& mesh, const MeshAttributeHandle<double>& vertex_attribute_handle)
    : TriangleAutodiffFunction(mesh, vertex_attribute_handle)
{}

TriAMIPS::~TriAMIPS() = default;

using DScalar = typename AutodiffFunction::DScalar;
using DSVec2 = Eigen::Vector2<DScalar>;
using DSVec3 = Eigen::Vector3<DScalar>;
DScalar TriAMIPS::eval(const simplex::Simplex& domain_simplex, const std::array<DSVec, 3>& coords)
    const
{
    ...
    default: throw std::runtime_error("TriAMIPS only supports 2D and 3D meshes");
    }
}
