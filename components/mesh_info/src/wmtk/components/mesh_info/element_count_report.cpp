...
nlohmann::json element_count_report_named(const wmtk::components::multimesh::MeshCollection& meshes)
{
    nlohmann::json out;
    for (const auto& [name, mesh] : meshes.all_meshes()) {
        out[name] = wmtk::components::mesh_info::element_count_report_named(mesh);
    }
    return out;
}
} // namespace wmtk::components::mesh_info
