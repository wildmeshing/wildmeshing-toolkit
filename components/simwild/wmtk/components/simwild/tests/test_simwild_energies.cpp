...
const std::string test_groups = "[simwild][2D][energies]";
#ifdef NDEBUG
...
std::shared_ptr<tri::SimWildMeshTri> init_optimization_tests(
    Parameters& params,
    double input_scale = 1.0)
{
    ...
    auto mesh = std::make_shared<tri::SimWildMeshTri>(params, params.eps);
...
TEST_CASE("scale-invariance", test_groups + test_release_only)
{
    ...
    auto collect_energies = [](double scale) {
        ...
        tri::SimWildMeshTri& mesh = *mesh_ptr;
