...
auto wmtk::extract_samples(
    const size_t width,
    const size_t height,
    const std::vector<float>& buffer,
    const float sx_,
    const float sy_) -> BicubicVector
{
    BicubicVector samples;
...
auto wmtk::make_samples_to_bicubic_coeffs_operator() -> BicubicMatrix
{
    BicubicMatrix ope;
    Eigen::Index row = 0;
...
auto wmtk::eval_bicubic_coeffs(const BicubicVector& coeffs, const float sx, const float sy) -> float
{
