...
std::vector<int> cyclic_order(const std::vector<Vector2r>& V)
{
    ...
    std::vector<std::array<int, 3>>& local_e_on_input)
{
    if (in_stack[v] == 1) {
        ...
        const int begin_idx = in_stack_idx[v].back();
        const int end_idx = stack.size() - 1;
        ...
        if (new_poly) {
            ...
            if (end_idx - begin_idx == 2) {
                ...
                int centroid_idx = V_pos.size();
                for (int i = begin_idx; i < end_idx + 1; ++i) {
                    centroid = centroid + V_pos[stack[i]];
                    int next_stack_idx = (i == end_idx) ? begin_idx : i + 1;
    ...
    std::map<int, int> global_to_local_idx;
    ...
    int base_idx = out_vec.size() - 1;

    auto cyclic_order_idx = cyclic_order(out_vec);

    // get all ccw vertices
    std::vector<int> ccw_vertices;
    std::vector<Vector2r> ccw_vec;
    std::vector<int> ccw_local_idx;

    int start_idx = -1;
    for (int i = 0; i < cyclic_order_idx.size(); ++i) {
    ...
    int colinear_out_idx = -1;
    int iter = (start_idx + 1) % out_vec.size();
    ...
    for (int i = 0; i < ccw_vertices.size(); ++i) {
    ...
    std::vector<std::array<int, 3>>& local_e_on_input)
{
    ...
    std::vector<std::vector<int>> in_stack_idx(VV.size());
...
void edge_insertion(
    ...
    std::vector<std::array<int, 3>>& FV_new,
    std::vector<std::array<int, 3>>& local_e_on_input)
{
    // code is assuming edgemesh doesn't have duplicated vertices

    MatrixXi EV_tmp = E_edge;

    // hack here for double input
    MatrixXd V_edge_double = V_edge;
    ...
    for (int i = 0; i < V_edge_tmp.rows(); ++i) {
        for (int j = 0; j < V_edge_tmp.cols(); ++j) {
            V_edge(i, j) = Rational(V_edge_tmp(i, j));
        }
    }

    std::vector<std::array<int, 2>> EV;
    for (int i = 0; i < EV_in.rows(); ++i) {
    ...
    //     trimesh.register_attribute<int>("segment_index", PrimitiveType::Vertex, 1, false, -1);
    // auto segment_index_accessor = trimesh.create_accessor<int>(segment_index_handle);
    ...
    std::vector<int> v_map_seg_to_tri(V_edge.rows());

    const auto& vs = trimesh.get_all(PrimitiveType::Vertex);
    for (int i = 0; i < vs.size(); ++i) {
        int seg_idx = segment_index_accessor.const_scalar_attribute(vs[i]);
    ...
    for (int i = 0; i < EV.size(); ++i) {
        const int idx0 = EV[i][0];
        const int idx1 = EV[i][1];
        ...
        for (int j = 0; j < segments.size(); ++j) {
            ...
            if (is_colinear(s.p0, s.p1, seg.p0, seg.p1)) {
                ...
                std::vector<std::pair<int, Vector2r>> all_points;
                ...
                auto comp = [](const std::pair<int, Vector2r>& v0,
                               const std::pair<int, Vector2r>& v1) {
                ...
                auto equal = [](const std::pair<int, Vector2r>& v0,
                                const std::pair<int, Vector2r>& v1) {
                ...
                if (end_idx != all_points.size() - 1) {
                    ...
                    for (int k = end_idx + 1; k < all_points.size() - 1; ++k) {
            ...
            for (int k = 0; k < s.points_on_segment.size(); ++k) {
            ...
            for (int k = 0; k < seg.points_on_segment.size(); ++k) {
    ...
    auto comp = [](const std::pair<int, Vector2r>& v0, const std::pair<int, Vector2r>& v1) {
        if (v0.second[0] == v1.second[0]) {
            return v0.second[1] < v1.second[1];
        }
        return v0.second[0] < v1.second[0];
    ...
    std::vector<std::vector<std::tuple<int, bool, bool>>> VV(v_final.size());

    for (int i = 0; i < segments.size(); ++i) {
        if (segments[i].deprecated) {
            continue;
        }
        for (int j = 0; j < segments[i].points_on_segment.size() - 1; ++j) {
