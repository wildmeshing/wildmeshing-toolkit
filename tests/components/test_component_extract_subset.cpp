...
bool is_manifold_2d(const wmtk::TriMesh& tm)
{
    ...
    for (long vid = 0; vid < vertices.size(); ++vid) {
        // wmtk::simplex::SimplexCollection sc = wmtk::simplex::top_dimension_cofaces(tm,
        // wmtk::Simplex::vertex(vertices[vid]));

        // find all faces that are connected to the vertex
        // TODO: replace adj_faces_of_vertex() with better implementation
            ...
            for (wmtk::Tuple edgeTuple : edgeList) {
                ...
                if (!tm.simplices_are_equal(
                    ...
                    !tm.simplices_are_equal(
                        ...
                        wmtk::Simplex::vertex(vertices[vid]))) {
                    // TODO: replace find_edge_index() with better implementation
...
bool is_disk(const wmtk::TetMesh& tm, std::set<long> index_set)
{
    ...
    //     std::cout << std::endl;
    // }
    std::vector<wmtk::Tuple> all_faces = tm.get_all(wmtk::PrimitiveType::Face);
    std::set<long> edge_indexs_in_link;
    std::set<long> vertex_indexs_in_link;
    for (long index : index_set) {
        wmtk::Tuple face = all_faces[index];
        std::vector<wmtk::Tuple> edgeList = wmtk::simplex::faces_single_dimension(
            tm,
            wmtk::Simplex::face(face),
            wmtk::PrimitiveType::Edge);
        std::vector<wmtk::Tuple> vertexList = wmtk::simplex::faces_single_dimension(
            tm,
            wmtk::Simplex::face(face),
            wmtk::PrimitiveType::Vertex);
        for (wmtk::Tuple vertex : vertexList) {
            vertex_indexs_in_link.insert(wmtk::components::internal::find_vertex_index(tm, vertex));
        }
        for (wmtk::Tuple edge : edgeList) {
...
bool is_manifold_3d(const wmtk::TetMesh& tm)
{
    ...
    for (long vid = 0; vid < vertices.size(); ++vid) {
        ...
        for (long fid : adj_tets) {
            ...
            if (!find_link_face) throw std::runtime_error("Link face not found!");
        }
    }
...
void random_trimesh_test_executor(const wmtk::TriMesh& m, const unsigned long test_size)
{
    ...
    long top_dimen_count = m.get_all(wmtk::PrimitiveType::Face).size();
    ...
    for (size_t i = 0; i < test_size; ++i) {
        ...
        bool after = is_manifold_2d(*dynamic_cast<wmtk::TriMesh*>(new_tm.get()));
...
TEST_CASE("component_3+4_test_case", "[components][extract_subset][2D][manual]")
{
    ...
    std::vector<int> tag_vector(tm.get_all(wmtk::PrimitiveType::Face).size(), 0);
    std::vector<int> id = {0,  1,  2,  3,  5,  6,  7,  8,  10, 11, 12, 25, 26, 29,
                           30, 31, 32, 33, 34, 36, 37, 38, 39, 40, 42, 43, 44, 45};
    for (int i : id) tag_vector[i] = 1;
    std::cout << "Before: nb_vertex = " << tm.capacity(wmtk::PrimitiveType::Vertex)
              << std::endl; // 30 -> 25
    std::cout << "Before: nb_face = " << tm.capacity(wmtk::PrimitiveType::Face)
              << std::endl; // 46 -> 28
    ...
    CHECK(new_tm->capacity(wmtk::PrimitiveType::Vertex) == 31);
...
TEST_CASE("2_non_manifold_vertices", "[components][extract_subset][3D][manual][3]")
{
    ...
    tets.row(2) << 1, 4, 5, 6;
    tm.initialize(tets);
    std::cout << "Before: # of vertices = " << tm.get_all(wmtk::PrimitiveType::Vertex).size()
              << std::endl;
    bool before = is_manifold_3d(tm);
    std::cout << "Before: manifold = " << before << std::endl;
    std::vector<int> tag_vector = {1, 1, 1};
    std::unique_ptr<wmtk::Mesh> topo_tm = wmtk::components::extract_subset(tm, tag_vector, false);
    std::cout << "After, # of vertices = " << topo_tm->get_all(wmtk::PrimitiveType::Vertex).size()
              << std::endl;
