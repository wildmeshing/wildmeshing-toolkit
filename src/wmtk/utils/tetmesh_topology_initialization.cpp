...
tetmesh_topology_initialization(Eigen::Ref<const RowVectors4l> T)
{
    ...
    // topology relations
    {
        ...
        for (int t = 0; t < T.rows(); ++t) {
            for (int i = 0; i < 4; ++i) {
                // v1 v2 v3 f ei
                long x = T(t, static_cast<long>(autogen::auto_3d_faces[i][0]));
                long y = T(t, static_cast<long>(autogen::auto_3d_faces[i][1]));
                long z = T(t, static_cast<long>(autogen::auto_3d_faces[i][2]));
                if (x > y) std::swap(x, y);
                if (y > z) std::swap(y, z);
                if (x > y) std::swap(x, y);
    ...
    // topology relations
    {
        ...
        for (int t = 0; t < T.rows(); ++t) {
            for (int i = 0; i < 6; ++i) {
                // v1 v2 f ei
                long x = T(t, static_cast<long>(autogen::auto_3d_edges[i][0]));
                long y = T(t, static_cast<long>(autogen::auto_3d_edges[i][1]));
                if (x > y) std::swap(x, y);
