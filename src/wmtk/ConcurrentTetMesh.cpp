...
bool wmtk::ConcurrentTetMesh::try_set_vertex_mutex_two_ring_vid(
    ...
    std::vector<size_t>& mutex_release_stack)
{
    for (auto v_one_ring : get_one_ring_vids_for_vertex(v)) {
        if (vector_contains(mutex_release_stack, v_one_ring)) continue;
        if (try_set_vertex_mutex(v_one_ring)) {
            mutex_release_stack.push_back(v_one_ring);
            for (auto v_two_ring : get_one_ring_vids_for_vertex(v_one_ring)) {
    ...
    std::vector<size_t>& mutex_release_stack)
{
    ...
    // try v1
    if (try_set_vertex_mutex(v1)) {
        mutex_release_stack.push_back(v1);
    } else {
        release_flag = true;
    }
    if (release_flag) {
    ...
    // try v2
    if (!vector_contains(mutex_release_stack, v2)) {
        if (try_set_vertex_mutex(v2)) {
            mutex_release_stack.push_back(v2);
        } else {
            release_flag = true;
        }
    }
    ...
    if (!vector_contains(mutex_release_stack, v3)) {
        if (try_set_vertex_mutex(v3)) {
            mutex_release_stack.push_back(v3);
        } else {
            release_flag = true;
        }
    }
    ...
    std::vector<size_t>& mutex_release_stack)
{
    if (!vector_contains(mutex_release_stack, v.vid(*this))) {
        if (try_set_vertex_mutex(v)) {
            mutex_release_stack.push_back(v.vid(*this));
            for (auto v_one_ring : get_one_ring_vids_for_vertex(v.vid(*this))) {
                if (!vector_contains(mutex_release_stack, v_one_ring)) {
                    if (try_set_vertex_mutex(v_one_ring)) {
                        mutex_release_stack.push_back(v_one_ring);
                    } else {
                        release_vertex_mutex_in_stack(mutex_release_stack);
