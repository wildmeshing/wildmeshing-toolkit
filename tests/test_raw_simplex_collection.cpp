...
TEST_CASE("raw_simplex_collection_sorting", "[raw_simplex_collection]")
{
    ...
    SimplexCollection raw_simplex_collection;
    for (const Tuple& t : vertices) {
...
TEST_CASE("raw_simplex_collection_set_operations", "[raw_simplex_collection]")
{
    ...
    sc2.add(Vertex(1));
    {
        auto inters = SimplexCollection::get_intersection(sc1, sc2);
    ...
    SimplexCollection sc_union1 = SimplexCollection::get_union(sc1, sc2);
    ...
    SimplexCollection sc_union2 = SimplexCollection::get_union(sc1, sc2);
    CHECK(SimplexCollection::are_simplex_collections_equal(sc_union1, sc_union2));

    SimplexCollection sc_inter = SimplexCollection::get_intersection(sc1, sc2);
...
TEST_CASE("raw_simplex_faces", "[raw_simplex_collection]")
{
    ...
    {
        Tet tet(0, 1, 2, 3);
        SimplexCollection tet_faces = SimplexCollection::faces_from_simplex(tet);
        ...
        SimplexCollection tri_faces = SimplexCollection::faces_from_simplex(tri);
        ...
        SimplexCollection edge_faces = SimplexCollection::faces_from_simplex(edge);
        ...
        CHECK((SimplexCollection::faces_from_simplex(vertex)).size() == 0);

        auto opp_edge = tet.opposite_edge(edge);
        SimplexCollection opp_edge_faces = SimplexCollection::faces_from_simplex(opp_edge);
        ...
        CHECK((SimplexCollection::faces_from_simplex(opp_vertex)).size() == 0);
    }
    SECTION("with_mesh")
    {
        using Tuple = TriMesh::Tuple;
        ...
        for (const Tuple& t : m.get_edges()) {
            ...
            SimplexCollection s_faces = SimplexCollection::faces_from_simplex(s);
            CHECK(s_faces.size() == 6);
            CHECK(s_faces.contains(Vertex(-1)));

            auto opposite_edge = s.opposite_edge(t.vid(m));
            auto oef = SimplexCollection::faces_from_simplex(opposite_edge);
